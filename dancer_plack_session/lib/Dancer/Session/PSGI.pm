...
sub create {
    print STDERR "DEBUG: create in " . __PACKAGE__ . "\n";
...
sub retrieve {
    ...
    print STDERR "DEBUG: retrieve from local module version\n";
...
sub flush {
    ...
    print STDERR "DEBUG: write with local mod version \n";

    map {$session->{$_} = $self->{$_}} keys %$self;

    print STDERR "DEBUG: write output: " . Dumper( $session ) . "\n";
